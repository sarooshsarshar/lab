...
if (typeof Storage !== "undefined") {
	if (localStorage.getItem("dore-direction")) {
		dir = localStorage.getItem("dore-direction");
	}
}

const changeSelect2 = function (index, element) {
	const ingredientChange = $("select.ingredient__change");
	const ingredientSecChange = $("select.ingredient__sec");
	const ingredientBranchChange = $("select.ingredient__branch");
	const ingredientUnitChange = $("select.ingredient__unit");

	$(element).on("select2:select", function (e) {
		const targetEle = $(e.target).find("option:selected");
		const parentId = targetEle.attr("data-parent");
		const unitId = targetEle.attr("data-unit");

		if (e.params.data.disabled === undefined) {
			$([
				ingredientUnitChange[index],
				ingredientBranchChange[index],
			]).removeAttr("disabled");
		} else {
			$([ingredientSecChange[index], ingredientChange[index]]).val(
				e.params.data.id
			);
			$(ingredientUnitChange[index]).val(unitId);
			$(ingredientBranchChange[index]).val(parentId);
			$([ingredientUnitChange[index], ingredientBranchChange[index]]).attr(
				"disabled",
				"disabled"
			);
			$([
				ingredientSecChange[index],
...
function select2Init() {
	...
	if ($().select2) {
		...
		$("select.dynamic__select").select2({
			theme: "bootstrap",
			placeholder: "",
		...
		$("select.dynamicnottags__select").select2({
			theme: "bootstrap",
			placeholder: "",
			maximumSelectionSize: 6,
			containerCssClass: ":all:",
		...
		$.each(ingredientChange, changeSelect2);
		$.each(ingredientSecChange, changeSelect2);
	}
}
