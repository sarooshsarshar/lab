...
(function ($) {
	...
	document.addEventListener("DOMContentLoaded", onStyleCompleteDelayed);
	...
	$("[data-section=ingredient]").on("click", ".delete__btn", function (event) {
		console.log($(this));

		$(this).closest("[data-row=ingredient]:not(:first-of-type)").remove();
	});
	...
	$('[data-add="receipe"]').click(function (event) {
		const target = $(this).attr("data-target");
		const targetHTML = $(this).attr("data-html");

		try {
			$("select.dynamic__select.select2-hidden-accessible").select2("destroy");
			$("select.dynamicnottags__select.select2-hidden-accessible").select2(
				"destroy"
			);
		} catch (error) {}
		const html = $(targetHTML).html();

		$(target).append(`<div class="row" data-row="ingredient">${html}</div>`);
